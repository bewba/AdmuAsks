...
<div
	class={`h-screen w-full flex flex-col transition-colors duration-200 ${$isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}
>
	...
	<main class="flex flex-col flex-1 w-full mx-auto px-12 py-4 gap-4 overflow-hidden">
		<div
			...
			class={`flex-1 w-full overflow-y-auto px-1 py-2 space-y-4 rounded-lg shadow-inner ${$isDarkMode ? 'bg-gray-800' : 'bg-white'}`}
		>
			{#each $messages as msg}
				<div
					class={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} items-start gap-2`}
				>
					{#if msg.role === 'assistant'}
						<div class="text-xl pt-1">🤖</div>
					{/if}
					<div
						class={`p-4 rounded-2xl max-w-[80%] shadow 
              ${
								msg.role === 'user'
			...
			{#if loading}
				<div class="flex justify-start ml-2">
					<div
						class={`p-4 rounded-2xl shadow-sm animate-pulse ${$isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}
					>
			...
			<div class="flex flex-wrap gap-3 mb-4">
				{#each samplePrompts as prompt}
					<button
						...
						class={`px-4 py-2 rounded-full shadow-sm text-sm font-medium transition-all duration-200
		...
		<div class="sticky bottom-0 bg-inherit z-10 pt-2">
			<form
				...
				class={`flex flex-col gap-4 p-4 rounded-lg shadow-lg ${$isDarkMode ? 'bg-gray-800' : 'bg-white'}`}
			>
				<textarea
              ...
              ${$isDarkMode ? 'bg-gray-700 text-white border border-gray-600 placeholder-gray-400' : 'bg-gray-100 text-gray-900 border border-gray-300 placeholder-gray-500'}`}
				...
				<button
					type="submit"
					class="bg-[#006937] hover:bg-[#005128] text-white py-3 px-6 rounded-xl font-semibold text-base md:text-lg transition-all duration-200
	...
	{#if showModal}
		...
		>
			<div
				class="bg-white dark:bg-gray-800 w-full max-w-2xl p-6 sm:p-8 rounded-2xl shadow-xl relative animate-fadeIn max-h-[90vh] overflow-y-auto"
			>
				<button
					class="absolute top-3 right-4 text-xl font-bold hover:text-red-500 cursor-pointer"
				...
				<div class="space-y-4 text-sm leading-relaxed">
					<div>
						...
						<ul class="list-disc ml-5">
							<li>
								This Service is intended to provide helpful responses based on the Lasallian student
								handbook.
							</li>
							<li>
					...
					<p>
						If you have questions or concerns, contact me here: <strong
							>hans_lumagui@dlsu.edu.ph</strong
						>
						or <strong>lumaguihans0123@gmail.com</strong>
					</p>
					<strong class="italic text-xs"
						>Disclaimer: Responses were created by generative AI and responses may be inaccurate.</strong
					><br />
					<strong class="italic text-xs"
						>Disclaimer: Responses are not representative of the creator.</strong
					>
					<br />
					<strong class="italic text-xs"
						>Disclaimer: THIS IS NOT AN OFFICIAL ATENEO DE MANILA UNIVERSITY CHATBOT.</strong
					>
				</div>
			</div>
		</div>
		...
		>
			...
			>
				...
				<div class="space-y-4 text-sm leading-relaxed">
					...
					<p class="italic">
						Have an app idea or a suggestion for the website? lmk, i'd love to hear from u!
					</p>
				</div>
			</div>
		</div>
...
{#if outOfApiCalls}
	...
	>
		<div
			class="bg-white dark:bg-gray-800 max-w-md w-full p-6 rounded-2xl shadow-xl relative animate-fadeIn"
		>
			...
			<div class="mt-4 text-right">
				<button
					class="px-4 py-2 bg-[#006937] text-white rounded hover:bg-[#005128]"
